...
async function createGithubRepo(repoName, files) {
  // Step 1: Create repo with auto_init so it has a commit
  var createRes = await ghFetch('/user/repos', {
    method: 'POST',
    body: JSON.stringify({
      name: repoName,
      private: false,
  ...
  var repoCreated = true;
  if (!createRes.ok) {
    var err = await createRes.json();
    if (err.errors && err.errors[0] && err.errors[0].message === 'name already exists on this account') {
      repoCreated = false; // Repo exists, we'll update it
    } else {
      throw new Error('GitHub repo creation failed: ' + JSON.stringify(err));
    }
  }

  // Step 2: Wait a moment for GitHub to initialize the repo
  if (repoCreated) {
    await new Promise(function(resolve) { setTimeout(resolve, 2000); });
  }

  // Step 3: Get the current main branch ref
  var refRes = await ghFetch('/repos/' + GITHUB_USER + '/' + repoName + '/git/ref/heads/main');
  var parentSha = null;
  if (refRes.ok) {
  ...
  for (var i = 0; i < fileEntries.length; i++) {
    ...
    var blobRes = await ghFetch('/repos/' + GITHUB_USER + '/' + repoName + '/git/blobs', {
      method: 'POST',
      body: JSON.stringify({ content: content, encoding: 'utf-8' }),
    });

    if (!blobRes.ok) {
      var blobErr = await blobRes.text();
      throw new Error('Failed to create blob for ' + filePath + ': ' + blobErr);
    }
  ...
  // Add .gitignore
  var giRes = await ghFetch('/repos/' + GITHUB_USER + '/' + repoName + '/git/blobs', {
    method: 'POST',
    body: JSON.stringify({ content: 'node_modules\n.next\n.env\n.env.local\n', encoding: 'utf-8' }),
  });
  var giBlob = await giRes.json();
  treeItems.push({ path: '.gitignore', mode: '100644', type: 'blob', sha: giBlob.sha });

  // Step 5: Create tree
  var treeBody = { tree: treeItems };
  var treeRes = await ghFetch('/repos/' + GITHUB_USER + '/' + repoName + '/git/trees', {
    method: 'POST',
  ...
  // Step 6: Create commit with parent if exists
  var commitBody = { message: 'Generated by Kumo AI', tree: tree.sha };
  if (parentSha) {
    commitBody.parents = [parentSha];
  }

  var commitRes = await ghFetch('/repos/' + GITHUB_USER + '/' + repoName + '/git/commits', {
    method: 'POST',
  ...
  // Step 7: Update the main branch ref
  if (parentSha) {
    await ghFetch('/repos/' + GITHUB_USER + '/' + repoName + '/git/refs/heads/main', {
      method: 'PATCH',
      body: JSON.stringify({ sha: commit.sha, force: true }),
    });
  } else {
    var newRefRes = await ghFetch('/repos/' + GITHUB_USER + '/' + repoName + '/git/refs', {
      method: 'POST',
