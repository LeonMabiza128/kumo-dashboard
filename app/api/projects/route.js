...
export async function GET() {
  ...
  try {
    const apps = await listApplications().catch(function() { return []; });
    const enriched = (Array.isArray(apps) ? apps : []).map(function(app) {
      return {
        uuid: app.uuid,
...
export async function POST(req) {
  ...
  try {
    ...
    return NextResponse.json({ project: project, application: app });
  } catch (err) {
...
function detectFramework(app) {
  var repo = (app.git_repository || '').toLowerCase();
  var bp = (app.build_pack || '').toLowerCase();
